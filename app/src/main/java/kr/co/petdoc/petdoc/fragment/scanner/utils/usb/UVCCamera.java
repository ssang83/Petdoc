...
import com.aramhuvis.lite.base.Log;
...
public class UVCCamera {
	...
	protected int mPanTiltMin, mPanTiltMax, mPanTiltDef;
    // until here
     ...
     * @param texture
     */
    public void setPreviewDisplay(final SurfaceTexture texture) {	// API >= 11
...
// 			   Log.d(TAG, "setZoom:zoom=" + zoom + " ,value=" + z);
	...
	//BANJ	get & set original value
	public synchronized int getZoom_original() {
		return nativeGetZoom(mNativePtr);
	}

	public synchronized int getRoll_original() {
		return nativeGetRoll(mNativePtr);
	}

	/*
	public synchronized int getPanTilt_original() {
		return nativeGetPanTilt(mNativePtr);
	}
	*/

	public synchronized void setZoom_original(final int zoom) {
		if (mNativePtr != 0) {
			nativeSetZoom(mNativePtr, zoom);
		}
	}

	public synchronized void setRoll_original(final int roll) {
		if (mNativePtr != 0) {
			nativeSetRoll(mNativePtr, roll);
		}
	}

	public synchronized void setPanTilt_original(final long pan, final long tilt) {
		if (mNativePtr != 0) {
			nativeSetPanTilt(mNativePtr, pan, tilt);
		}
	}
    	...
    	if (mNativePtr != 0) {
    		if ((mControlSupports == 0) || (mProcSupports == 0)) {
    	    	...
    	    	if ((mControlSupports != 0) && (mProcSupports != 0)) {
					...
					nativeUpdateWhiteBlanceLimit(mNativePtr);
					nativeUpdateFocusLimit(mNativePtr);
					//BANJ ADD ROLL&PANTILT CONTROL
					nativeUpdateRollLimit(mNativePtr);
					nativeUpdatePanTiltLimit(mNativePtr);
    	    	}
    	    	if (DEBUG) {
	...
	private final String getUSBFSName(final UsbControlBlock ctrlBlock) {
		...
		if (TextUtils.isEmpty(result)) {
			Log.v(TAG, "failed to get USBFS path, try to use default path:" + name);
    ...
    private static final native int nativeConnect(long id_camera, int venderId, int productId, int fileDescriptor, int busNum, int devAddr, String usbfs);
...
//**********************************************************************
	...
	private static final native int nativeSetRoll(final long id_camera, final int zoom);
	private static final native int nativeGetRoll(final long id_camera);

	private final native int nativeUpdatePanTiltLimit(final long id_camera);
	private static final native int nativeSetPanTilt(final long id_camera, final long pan, final long tilt);
	//private static final native int nativeGetPanTilt(final long id_camera);
}
