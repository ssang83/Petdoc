...
class RecyclerViewSwipeDecorator private constructor() {
    ...
    private var swipeLeftBackgroundColor: Int
    private var swipeLeftActionIconId: Int
    private var swipeLeftActionIconTint: Int?
    private var swipeRightBackgroundColor: Int
    private var swipeRightActionIconId: Int
    private var swipeRightActionIconTint: Int?
        ...
        try {
            ...
            if (dX > 0) {
                ...
                canvas.clipRect(
                    ...
                    viewHolder.itemView.getLeft() + dX.toInt(),
                ...
                if (swipeRightActionIconId != 0 && dX > iconHorizontalMargin) {
                    ...
                    if (icon != null) {
                        ...
                        val top: Int =
                            viewHolder.itemView.getTop() + ((viewHolder.itemView.getBottom() - viewHolder.itemView.getTop()) / 2 - halfIcon)
                        icon.setBounds(
                            viewHolder.itemView.getLeft() + iconHorizontalMargin,
                            top,
                            viewHolder.itemView.getLeft() + iconHorizontalMargin + icon.intrinsicWidth,
                ...
                if (mSwipeRightText != null && mSwipeRightText!!.length > 0 && dX > iconHorizontalMargin + iconSize) {
                    ...
                    val textTop = (viewHolder.itemView.getTop() + (viewHolder.itemView.getBottom() - viewHolder.itemView.getTop()) / 2.0 + textPaint.textSize / 2)

                    canvas.drawText(
                        mSwipeRightText!!,
                        (viewHolder.itemView.getLeft() + iconHorizontalMargin + iconSize + if (iconSize > 0) iconHorizontalMargin / 2 else 0).toFloat(),
                        textTop.toFloat(),
                        textPaint
                    )
                }
                ...
                var iconSize = 0
                var imgLeft: Int = viewHolder.itemView.getRight()
                ...
                if (mSwipeLeftText != null && mSwipeLeftText!!.length > 0 && dX < -iconHorizontalMargin - iconSize) {
                    ...
                    canvas.drawText(
                        mSwipeLeftText!!,
                        imgLeft - width - if (imgLeft == viewHolder.itemView.getRight()) iconHorizontalMargin else iconHorizontalMargin / 2,
                        textTop.toFloat(),
                        textPaint
                    )
                }
    ...
    class Builder(
         ...
         * @return This instance of @RecyclerViewSwipeDecorator.Builder
         */
        fun setSwipeRightLabelTextSize(
            unit: Int,
            size: Float
        ): Builder {
