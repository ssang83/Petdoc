...
/*		if (listener == null)
	...
	public synchronized void requestPermission(final UsbDevice device) {
		...
		Log.v(TAG, "mPermissionIntent : " + mPermissionIntent);
		Log.v(TAG, "device : " + device);
		Log.v(TAG, "mUsbManager.hasPermission(device) : " + mUsbManager.hasPermission(device));
		
		if (mPermissionIntent != null) {
			if (device != null) {
				Log.v("USB", "mUsbManager.hasPermission(device) : " + mUsbManager.hasPermission(device));
	...
	public static final class UsbControlBlock {
		...
		public UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			...
			if (mConnection != null) {
				...
				Log.v(TAG, "could not connect to device " + name);
			}
		}
