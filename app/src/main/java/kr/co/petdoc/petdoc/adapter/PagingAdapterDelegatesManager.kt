...
class PagingAdapterDelegatesManager<T: PagingItem> {

    private val arrayList = ArrayList<Any>()

    private var delegates: SparseArrayCompat<PagingAdapterDelegate<T>> = SparseArrayCompat()

    /**
     * Adds an [AdapterDelegate] to the current list of delegates and returns this
     * [PagingAdapterDelegatesManager].
     */
    fun addAdapterDelegate(delegate: PagingAdapterDelegate<T>) {
     ...
     * [PagingAdapterDelegatesManager].
     */
    fun removeAdapterDelegate(delegate: PagingAdapterDelegate<T>) {
    ...
    fun getItemViewType(item: T): Int {
        ...
        throw NullPointerException("Delegate not found for item at position")
    }
    ...
    fun onBindViewHolder(
        ...
        delegate.onBindViewHolder(item, position, viewHolder, payloads ?: arrayList)
    }
    ...
    fun onBindViewHolder(item: T, position: Int, viewHolder: RecyclerView.ViewHolder?) =
        onBindViewHolder(item, position, viewHolder, arrayList)
     ...
     */
    fun getDelegateForViewType(viewType: Int): PagingAdapterDelegate<T>? = delegates.get(viewType)
     ...
     */
    fun getViewType(delegate: PagingAdapterDelegate<T>): Int {
        val index = delegates.indexOfValue(delegate)
        if (index == -1) {
            return -1
        }
